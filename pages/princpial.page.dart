import 'package:flutter/material.dart'

class PrincipalPage extends StatefulWidget {
@Override

_PrincipalPageState createState() => _PrincipalPageState():

}

class _PrincipalPageState extends State<PrincipalPage> {
@Override
Widget build(BuildContext context){
return Scaffold (

appBar:  AppBar (

title: Text(""),
backgroundColor: Colors.grey[700],
elevation : 0

), //AppBar

body: Container(

padding: EdgeInsets.only(left: 20, top: 5, right: 20),
width: double.infinity.
height: double.infinity,
aligment: Aligment.topLeft,
decoration: BoxDecorarion(
gradient: LinearGradient(
begin: Aligment.topCenter,
end: Aligment.bottomCenter,
colors: [Colors.grey[700], Colors.grey[900], Colors.grey[900]] // completar

), // LinearGradient
), // decorator

child: SingleChildScrollView(

child: Column(
children: <Widget>[
  Text("Tocados recentemente", style: TextStyle(
  color: Colors.white,
  fontSize: 20
  ),), // TextStyle // Text
  SizedBox(height: 30),
  SingleChildScrollView(
  scrollDirection: Axis.horizontal,
  child: Row(
    children: <Widget>[
      Container(width: 90, height: 90, decoration: BoxDecorarion(color: Colors.white),),
      SizedBox(width: 20),
      Container(width: 90, height: 90, decoration: BoxDecorarion(color: Colors.white, borderRadius: borderRadius.all(Radius.circular(50))),),
      SizedBox(width: 20),
      Container(width: 90, height: 90, decoration: BoxDecorarion(color: Colors.white),),
      SizedBox(width: 20),
      Container(width: 90, height: 90, decoration: BoxDecorarion(color: Colors.white),),

    ], // Widget
  ), // row
  ), // SingleChildScrollView

   SizedBox(height: 40,),
                     Text("Feitos para Você" , style: TextStyle(
                    color: Colors.white,
                    fontSize: 20
                  ),), // TextStyle // Text
SizedBox(height: 30,),
                  SingleChildScrollView(
                    scrollDirection: Axis.horizontal,
                    child: Row(
                      children:<Widget>[

                          Container( width: 90, height: 90, decoration: BoxDecoration(color: Colors.white),),
                          SizedBox(width: 20,),
                          Container( width: 90, height: 90, decoration: BoxDecoration(color: Colors.white ,)),
                          SizedBox(width: 20,),
                          Container( width: 90, height: 90, decoration: BoxDecoration(color: Colors.white),),
                          SizedBox(width: 20,),
                          Container( width: 90, height: 90, decoration: BoxDecoration(color: Colors.white),),
                          SizedBox(width: 20,),
                          Container( width: 90, height: 90, decoration: BoxDecoration(color: Colors.white),),

                      ]
                    ),
                  ),
              ],
              ),
        ),


      ),

      bottomNavigationBar: BottomNavigationBar(
        currentIndex: 0,
        backgroundColor: Colors.grey[800],
        items: [
          BottomNavigationBarItem(
            icon: new Icon(Icons.home),
            title: new Text('Home')
          ),
           BottomNavigationBarItem(
            icon: new Icon(Icons.search , color: Colors.grey,),
            title: new Text('Buscar' , style: TextStyle(color: Colors.grey),)
          ),
          BottomNavigationBarItem(
            icon: new Icon(Icons.library_books, color: Colors.grey,),
            title: new Text('Biblioteca' ,style: TextStyle(color: Colors.grey), )
          ),
         
        ],
      ),
      
    );
  }
}